...
while (true)
{
    ...
    // method 1. define test case, rather than soliciting user for input
    ...
    // strategy: bookmark at index where value < target, then work towards the beginning with smaller numbers
        ...
        if (arr[i] < tar)
        {
            for (int j = 0; j < arr.Length; j++)
            {
                ...
                if (showVerboseComments) 
                    Console.Write($"     seeing if {arr[i]} + {arr[j]} == {tar}   ");
                if (arr[i] + arr[j] == tar)
                {
                    if (showVerboseComments) 
                        Console.WriteLine($"YES, returning [{i},{j}]");
